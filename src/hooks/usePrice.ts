...
export const PRICE_KEYS = {
  ...
  universe: (tradingDay: string) =>
    [...PRICE_KEYS.all, "universe", tradingDay] as const,
  eod: (symbol: string, tradingDay: string) =>
    [...PRICE_KEYS.all, "eod", symbol, tradingDay] as const,
  settlement: (tradingDay: string) =>
    [...PRICE_KEYS.all, "settlement", tradingDay] as const,
  ...
  eodCollection: (tradingDay: string) =>
    [...PRICE_KEYS.all, "collect", tradingDay] as const,
} as const;
...
export const useUniversePrices = (
  tradingDay: string,
  options?: Omit<
    UseQueryOptions<UniversePricesPayload, Error>,
    "queryKey" | "queryFn"
  >,
) => {
...
export const useEodPrice = (
  ...
  return useQuery<EODPriceResponse, Error>({
    ...
    enabled:
      !!symbol && !!tradingDay && (options?.enabled === undefined || options.enabled),
    ...options,
  });
};
...
export const useSettlementValidation = (
  tradingDay: string,
  options?: Omit<
    UseQueryOptions<SettlementPriceData[], Error>,
    "queryKey" | "queryFn"
  >,
) => {
