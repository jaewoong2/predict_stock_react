  ...
  return (
    <AuthGuard>
      <div className="container mx-auto px-4 py-6">
        <div className="mb-6">
          <h1 className="text-3xl font-bold text-gray-900 dark:text-white">관리자</h1>
          <p className="mt-2 text-gray-600 dark:text-gray-400">리워드/사용자/포인트 관리 (초안)</p>
        </div>
      ...
      <Card>
        ...
        <CardContent>
          {isLoading ? (
            ...
            <div className="py-8 text-center text-red-600 dark:text-red-400">카탈로그 로딩 실패</div>
          ) : (
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
              {items.map((i) => (
                <Card key={i.id} className="overflow-hidden">
                  <CardHeader>
                    <CardTitle className="truncate text-base" title={i.name}>{i.name}</CardTitle>
                  </CardHeader>
                  <CardContent>
                    <div className="mb-2 text-sm text-gray-500 dark:text-gray-400">{i.description}</div>
                    <div className="flex items-center justify-between text-sm">
                      <span className="font-semibold">{i.points_cost.toLocaleString()} P</span>
                      <span className="text-gray-500 dark:text-gray-400">재고 {i.stock_quantity ?? '-'}</span>
                    </div>
                    <div className="mt-2 text-sm">
                      {i.is_available ? (
                        <span className="text-green-600 dark:text-green-400">노출중</span>
                      ) : (
                        <span className="text-gray-500 dark:text-gray-400">비노출</span>
                      )}
                    </div>
                  </CardContent>
                </Card>
    ...
    <Card>
      ...
      <CardContent>
        {isLoading ? (
          ...
          <div className="py-8 text-center text-red-600 dark:text-red-400">목록을 불러오지 못했습니다.</div>
        ) : (
