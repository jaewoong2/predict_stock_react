...
export function HomeTopStrip({ activeTab = "home" }: HomeTopStripProps) {
  ...
  return (
    <div className="sticky top-0 z-10 -mx-4 mb-3 border-b bg-white/90 px-4 py-2 backdrop-blur dark:bg-black/60">
      {/* Indices row (placeholder values; wire actual indices later) */}
      <div className="mb-2 flex items-center justify-between text-xs">
        <div className="flex items-center gap-3">
          <IndexBadge name="나스닥" value="16,356" delta="+0.62%" />
          <IndexBadge name="다우" value="39,806" delta="-0.04%" negative />
          <IndexBadge name="S&P 500" value="5,308" delta="+0.12%" />
        </div>
