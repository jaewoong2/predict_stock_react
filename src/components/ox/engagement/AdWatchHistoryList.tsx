...
export function AdWatchHistoryList() {
  const { data, isLoading, error } = useAdWatchHistory({ limit: 20, offset: 0 });
    ...
    <Card>
      ...
      <CardContent>
        {isLoading ? (
          ...
          <div className="py-8 text-center text-gray-500 dark:text-gray-400">광고 시청 내역이 없습니다.</div>
        ) : (
              ...
              <div key={h.id} className="flex items-center justify-between rounded-md border p-3">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <div className="text-xs text-gray-500 dark:text-gray-400">{h.ad_unit_id}</div>
                  </div>
                </div>
                <div className="text-right text-sm">
                  <div className="flex items-center justify-end gap-2 text-gray-500 dark:text-gray-400">
                    <Clock className="h-3 w-3" />
                    {formatDistanceToNow(new Date(h.watched_at), { addSuffix: true, locale: ko })}
                  </div>
                  {h.points_earned ? (
                    <div className="font-semibold text-green-600 dark:text-green-400">+{h.points_earned}P</div>
                  ) : null}
                </div>
              </div>
            ))}
